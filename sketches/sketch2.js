const boxBreathingClock = function (p) {
  ...
  const lineColors = [
    p => p.color(56,127,117),
    p => p.color(123,188,182),
    p => p.color(152,228,217),
    p => p.color(168,189,191),
  ];
  ...
  p.draw = function () {
    ...
    // soft glow
    const c = lineColors[phase](p);
    p.drawingContext.shadowBlur = 18;
    p.drawingContext.shadowColor = p.color(p.red(c), p.green(c), p.blue(c), 180);

    // active edge
    p.noFill(); p.strokeWeight(8); p.stroke(c);
    const t = 0.5 - 0.5 * Math.cos(p.PI * progress); // easeInOut for smoothness
    switch (phase) {
      case 0: p.line(p.lerp(x1, x2, t), y1, x2, y1); break;
      case 1: p.line(x2, p.lerp(y1, y2, t), x2, y2); break;
      case 2: p.line(x1, y2, p.lerp(x2, x1, t), y2); break;
      case 3: p.line(x1, y1, x1, p.lerp(y2, y1, t)); break;
    }
    ...
    p.text(Math.ceil(4 - progress * 4), p.width / 2, cy + p.width * 0.02);
  };
